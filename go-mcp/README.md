# transportrest-transit-apis-mcp

[MCP](https://modelcontextprotocol.io) server exposing the TransportrestTransitApis SDK as
two agent tools — `transportrest-transit-apis_list` and `transportrest-transit-apis_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/transportrest-transit-apis-mcp)
make build

# 2. Provide credentials via the environment
export TRANSPORTREST_TRANSIT_APIS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user transportrest-transit-apis \
  -- /absolute/path/to/transportrest-transit-apis-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./transportrest-transit-apis-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// transportrest-transit-apis_list: first page of records
{ "entity": "arrival" }
{ "entity": "arrival", "query": { } }

// transportrest-transit-apis_load: one record by id
{ "entity": "stop", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/transportrest-transit-apis-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user transportrest-transit-apis \
     -- "$PWD"/dist/*/transportrest-transit-apis-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `transportrest-transit-apis_list` and `transportrest-transit-apis_load` tools now appear
   in new sessions. Ask the agent to *"list arrival using transportrest-transit-apis"*
   and it calls `transportrest-transit-apis_list` with `{"entity":"arrival"}`.
...
```sh
export TRANSPORTREST_TRANSIT_APIS_APIKEY=sk_live_xxx            # API key
export TRANSPORTREST_TRANSIT_APIS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `transportrest-transit-apis_list` tool
...
### Call the `transportrest-transit-apis_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `transportrest-transit-apis_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `transportrest-transit-apis_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TRANSPORTREST_TRANSIT_APIS_APIKEY` | API key sent with every request. |
| `TRANSPORTREST_TRANSIT_APIS_BASE` | Optional override of the API base URL. |

### Entities

The 7 entities valid as the `entity` argument:

arrival | departure | journey | location | radar | stop | trip

### Smoke test via HTTP (raw JSON-RPC)

```sh
./transportrest-transit-apis-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"transportrest-transit-apis_load","arguments":{"entity":"stop","query":{"id":1}}}}'
```
