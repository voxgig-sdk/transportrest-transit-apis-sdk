...
require("transportrest-transit-apis_types")
...
local TransportrestTransitApisSDK = {}
TransportrestTransitApisSDK.__index = TransportrestTransitApisSDK
...
TransportrestTransitApisSDK._make_feature = _make_feature


function TransportrestTransitApisSDK.new(options)
  local self = setmetatable({}, TransportrestTransitApisSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function TransportrestTransitApisSDK:options_map()
...
function TransportrestTransitApisSDK:get_utility()
  return Utility.copy(self._utility)
end


function TransportrestTransitApisSDK:get_root_ctx()
  return self._rootctx
end


function TransportrestTransitApisSDK:prepare(fetchargs)
...
function TransportrestTransitApisSDK:direct(fetchargs)
...
function TransportrestTransitApisSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function TransportrestTransitApisSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "TransportrestTransitApisSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function TransportrestTransitApisSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function TransportrestTransitApisSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "TransportrestTransitApisSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function TransportrestTransitApisSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = TransportrestTransitApisSDK.new(sdkopts)
  sdk.mode = "test"
...
return TransportrestTransitApisSDK
