...
local JourneyEntity = {}
JourneyEntity.__index = JourneyEntity


function JourneyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, JourneyEntity)
  self._name = "journey"
  self._client = client
...
function JourneyEntity:get_name()
  return self._name
end


function JourneyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return JourneyEntity.new(self._client, opts)
end
...
function JourneyEntity:mark_deleted()
  self._deleted = true
end


function JourneyEntity:deleted()
  return true == self._deleted
end


function JourneyEntity:data_set(args)
  if args ~= nil then
...
function JourneyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function JourneyEntity:match_set(args)
  if args ~= nil then
...
function JourneyEntity:match_get()
...
function JourneyEntity:stream(action, args, callopts)
...
---@param reqmatch JourneyListMatch
---@param ctrl? table
---@return Journey[]
---@return string? err
function JourneyEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function JourneyEntity:_run_op(ctx, post_done)
  local utility = self._utility
