...
local ArrivalEntity = {}
ArrivalEntity.__index = ArrivalEntity


function ArrivalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ArrivalEntity)
  self._name = "arrival"
  self._client = client
...
function ArrivalEntity:get_name()
  return self._name
end


function ArrivalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ArrivalEntity.new(self._client, opts)
end
...
function ArrivalEntity:mark_deleted()
  self._deleted = true
end


function ArrivalEntity:deleted()
  return true == self._deleted
end


function ArrivalEntity:data_set(args)
  if args ~= nil then
...
function ArrivalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ArrivalEntity:match_set(args)
  if args ~= nil then
...
function ArrivalEntity:match_get()
...
function ArrivalEntity:stream(action, args, callopts)
...
---@param reqmatch ArrivalListMatch
---@param ctrl? table
---@return Arrival[]
---@return string? err
function ArrivalEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ArrivalEntity:_run_op(ctx, post_done)
  local utility = self._utility
