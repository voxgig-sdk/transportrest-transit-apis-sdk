...
local RadarEntity = {}
RadarEntity.__index = RadarEntity


function RadarEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RadarEntity)
  self._name = "radar"
  self._client = client
...
function RadarEntity:get_name()
  return self._name
end


function RadarEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RadarEntity.new(self._client, opts)
end
...
function RadarEntity:mark_deleted()
  self._deleted = true
end


function RadarEntity:deleted()
  return true == self._deleted
end


function RadarEntity:data_set(args)
  if args ~= nil then
...
function RadarEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RadarEntity:match_set(args)
  if args ~= nil then
...
function RadarEntity:match_get()
...
function RadarEntity:stream(action, args, callopts)
...
function RadarEntity:_run_op(ctx, post_done)
  local utility = self._utility
