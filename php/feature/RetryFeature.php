...
// TransportrestTransitApis SDK retry feature
...
class TransportrestTransitApisRetryFeature extends TransportrestTransitApisBaseFeature
{
    ...
    public function init(TransportrestTransitApisContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TransportrestTransitApisContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TransportrestTransitApisContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TransportrestTransitApisContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
