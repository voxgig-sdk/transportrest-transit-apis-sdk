...
// TransportrestTransitApis SDK Arrival entity
...
class ArrivalEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ArrivalEntity($this->_client, $opts);
    }

    /**
     * @param Arrival|array $args Arrival data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = TransportrestTransitApisHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Arrival|array The current Arrival data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Arrival fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = TransportrestTransitApisHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Arrival fields).
     */
    ...
    /**
     * List Arrival items matching the given filter.
     *
     * @param ArrivalListMatch|array|null $reqmatch Match filter (any subset
     *   of Arrival fields) as an assoc-array; ArrivalListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Arrival[]|array A list of Arrival items as assoc-arrays at
     *   the SDK boundary; throws TransportrestTransitApisError on failure (item-5 convention).
     */
