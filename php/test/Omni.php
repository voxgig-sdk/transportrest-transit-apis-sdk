...
//    only - a typed TransportrestTransitApisContext there would make every ctx
...
final class TransportrestTransitApisOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TransportrestTransitApisContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new TransportrestTransitApisSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new TransportrestTransitApisResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new TransportrestTransitApisError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new TransportrestTransitApisResponse($clean['response']);
    ...
    public static function err_from_map($m): ?TransportrestTransitApisError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TransportrestTransitApisError(is_string($code) ? $code : '', $msg);
    }
