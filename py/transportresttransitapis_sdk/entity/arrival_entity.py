...
from transportresttransitapis_sdk.transportresttransitapis_types import (
    Arrival,
    ArrivalListMatch,
)
...
class ArrivalEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ArrivalEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Arrival]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Arrival().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
