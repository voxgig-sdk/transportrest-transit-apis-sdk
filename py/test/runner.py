# TransportrestTransitApis SDK test runner
...
class TransportrestTransitApisTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TransportrestTransitApisTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TransportrestTransitApisTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TransportrestTransitApisTestRunner.getenv("TRANSPORTREST_TRANSIT_APIS_TEST_LIVE")
        override = TransportrestTransitApisTestRunner.getenv("TRANSPORTREST_TRANSIT_APIS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TransportrestTransitApisTestRunner.getenv(key)
        ...
        explain = TransportrestTransitApisTestRunner.getenv("TRANSPORTREST_TRANSIT_APIS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if TransportrestTransitApisTestRunner._test_control is not None:
            return TransportrestTransitApisTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TransportrestTransitApisTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TransportrestTransitApisTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TransportrestTransitApisTestRunner.load_test_control()
...
def load_env_local():
    TransportrestTransitApisTestRunner.load_env_local()


def env_override(m):
    return TransportrestTransitApisTestRunner.env_override(m)


def entity_data(v):
    return TransportrestTransitApisTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TransportrestTransitApisTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TransportrestTransitApisTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TransportrestTransitApisTestRunner.load_test_control()


def live_client_options():
    return TransportrestTransitApisTestRunner.live_client_options()


def live_delay_ms():
    return TransportrestTransitApisTestRunner.live_delay_ms()
