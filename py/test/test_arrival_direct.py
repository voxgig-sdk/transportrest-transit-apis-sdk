...
class TestArrivalDirect:

    def test_should_direct_list_arrival(self):
        setup = _arrival_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-arrival", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["stop01"]:
...
def _arrival_direct_setup(mockres):
