...
class TestJourneyDirect:

    def test_should_direct_list_journey(self):
        setup = _journey_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-journey", "live" if setup["live"] else "unit")
        if _skip:
...
def _journey_direct_setup(mockres):
