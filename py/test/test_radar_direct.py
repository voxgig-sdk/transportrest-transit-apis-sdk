...
class TestRadarDirect:

    def test_should_direct_list_radar(self):
        setup = _radar_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-radar", "live" if setup["live"] else "unit")
        if _skip:
...
def _radar_direct_setup(mockres):
