...
func TestJourneyEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Journey(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Journey(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		journeyRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.journey")))
...
func journeyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TRANSPORTREST_TRANSIT_APIS_TEST_JOURNEY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TRANSPORTREST_TRANSIT_APIS_TEST_JOURNEY_ENTID"])
