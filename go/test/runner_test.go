...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TRANSPORTREST_TRANSIT_APIS_TEST_LIVE") == "TRUE" ||
		os.Getenv("TRANSPORTREST_TRANSIT_APIS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TRANSPORTREST_TRANSIT_APIS_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.TransportrestTransitApisSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.TransportrestTransitApisError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.TransportrestTransitApisSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.TransportrestTransitApisError{Msg: msg, Code: code}
}
