...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TRANSPORTRESTTRANSITAPIS_TEST_LIVE") == "TRUE" ||
		os.Getenv("TRANSPORTRESTTRANSITAPIS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TRANSPORTRESTTRANSITAPIS_TEST_EXPLAIN"); explain != "" {
