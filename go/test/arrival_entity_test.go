...
func TestArrivalEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Arrival(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Arrival(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set TRANSPORTREST_TRANSIT_APIS_TEST_ARRIVAL_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func arrivalBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TRANSPORTREST_TRANSIT_APIS_TEST_ARRIVAL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TRANSPORTREST_TRANSIT_APIS_TEST_ARRIVAL_ENTID"])
