...
import (
	...
	omni "github.com/voxgig-sdk/transportrest-transit-apis-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TransportrestTransitApisError); is {
				out := map[string]any{
					"name":    "TransportrestTransitApisError",
...
func omniCtx(arg any, client *sdk.TransportrestTransitApisSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TransportrestTransitApisSDK); is {
			client = live
