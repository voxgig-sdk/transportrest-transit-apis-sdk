...
class JourneyEntity
  ...
  def make
    opts = @_entopts.dup
    JourneyEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TransportrestTransitApisHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Journey, Hash] the current Journey data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = TransportrestTransitApisHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Journey fields)
  def match_get
  ...
  #
  # @param reqmatch [JourneyListMatch, Hash, nil] match filter (any subset of
  #   Journey fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Journey>, Array] the matching Journey items; raises TransportrestTransitApisError on failure
