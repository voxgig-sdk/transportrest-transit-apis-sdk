...
module TransportrestTransitApisOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if TransportrestTransitApisOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = TransportrestTransitApisOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| TransportrestTransitApisOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || TransportrestTransitApisOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = TransportrestTransitApisSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = TransportrestTransitApisError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { TransportrestTransitApisOmni.errify(err) }
