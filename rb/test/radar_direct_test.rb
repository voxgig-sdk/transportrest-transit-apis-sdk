...
class RadarDirectTest < Minitest::Test
  def test_direct_list_radar
    setup = radar_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-radar", setup[:live] ? "live" : "unit")
    if _should_skip
...
def radar_direct_setup(mockres)
