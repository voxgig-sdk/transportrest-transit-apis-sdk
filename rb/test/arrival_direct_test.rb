...
class ArrivalDirectTest < Minitest::Test
  def test_direct_list_arrival
    setup = arrival_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-arrival", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["stop01"].each do |_live_key|
...
def arrival_direct_setup(mockres)
  ...
  if live
    ...
    client = TransportrestTransitApisSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
