...
class JourneyDirectTest < Minitest::Test
  def test_direct_list_journey
    setup = journey_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-journey", setup[:live] ? "live" : "unit")
    if _should_skip
...
def journey_direct_setup(mockres)
