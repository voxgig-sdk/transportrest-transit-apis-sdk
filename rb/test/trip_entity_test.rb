...
class TripEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TRANSPORTREST_TRANSIT_APIS_TEST_TRIP_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    trip_ref01_data_dt0_load_result = Helpers.to_map(trip_ref01_data_dt0_loaded.respond_to?(:data_get) ? trip_ref01_data_dt0_loaded.data_get : trip_ref01_data_dt0_loaded)
...
def trip_basic_setup(extra)
  ...
  entid_env_raw = ENV["TRANSPORTREST_TRANSIT_APIS_TEST_TRIP_ENTID"]
