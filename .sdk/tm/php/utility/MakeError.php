...
class TransportrestTransitApisMakeError
{
    public static function call(?TransportrestTransitApisContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TransportrestTransitApisContext([], null);
        }
        $op = $ctx->op ?? new TransportrestTransitApisOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TransportrestTransitApisResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TransportrestTransitApisError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new TransportrestTransitApisError('', $msg, $ctx);
        ...
        if ($err instanceof TransportrestTransitApisError) {
