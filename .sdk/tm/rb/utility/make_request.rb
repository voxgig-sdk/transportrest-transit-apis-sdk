...
module TransportrestTransitApisUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TransportrestTransitApisResponse.new({})
    ...
    if fetch_err
      ...
      response = TransportrestTransitApisResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TransportrestTransitApisResponse.new(fetched)
    else
