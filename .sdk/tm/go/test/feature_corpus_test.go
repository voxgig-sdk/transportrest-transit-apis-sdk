...
//
// NewTransportrestTransitApisSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.TransportrestTransitApisSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewTransportrestTransitApisSDK(opts)
}
...
func fcCandidates(client *sdk.TransportrestTransitApisSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.TransportrestTransitApisSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.TransportrestTransitApisSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.TransportrestTransitApisError
