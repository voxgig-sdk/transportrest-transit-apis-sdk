# transportrest-transit-apis-cli

boru-driven command-line client **and** interactive REPL for the TransportrestTransitApis
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/transportrest-transit-apis-cli)
make build

# 2. See usage (words, entities, env vars)
./transportrest-transit-apis-cli --help

# 3. Provide credentials once, via the environment
export TRANSPORTREST_TRANSIT_APIS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./transportrest-transit-apis-cli list arrival
./transportrest-transit-apis-cli list departure

# 5. Override the API base URL for a single call
TRANSPORTREST_TRANSIT_APIS_BASE=https://api.example.com ./transportrest-transit-apis-cli list arrival

# 6. No arguments -> interactive REPL
./transportrest-transit-apis-cli
transportrest-transit-apis> list arrival
transportrest-transit-apis> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/transportrest-transit-apis-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/transportrest-transit-apis-cli list arrival
   ```
...
Run with no arguments to open a REPL (prompt `transportrest-transit-apis>`). Each line is
evaluated as its own boru expression:

```text
$ ./transportrest-transit-apis-cli
transportrest-transit-apis> list arrival
transportrest-transit-apis> /help
transportrest-transit-apis> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `arrival`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/transportrest-transit-apis-cli`. |
...
(`./transportrest-transit-apis-cli <expr>`) and interactively (the REPL), and expressions compose the
