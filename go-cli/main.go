// Command TransportrestTransitApis-cli is a CLI + REPL for the TransportrestTransitApis SDK,
...
const prompt = "transportrest-transit-apis"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "arrival departure journey location radar stop trip"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TRANSPORTREST_TRANSIT_APIS_APIKEY (api key), TRANSPORTREST_TRANSIT_APIS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TRANSPORTREST_TRANSIT_APIS_APIKEY carries the API key and
	// TRANSPORTREST_TRANSIT_APIS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TRANSPORTREST_TRANSIT_APIS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TRANSPORTREST_TRANSIT_APIS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTransportrestTransitApisSDK(opts)
