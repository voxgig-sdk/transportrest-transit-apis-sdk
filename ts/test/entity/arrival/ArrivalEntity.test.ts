...
describe('ArrivalEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'arrival.' + op, live)) return
    }
    ...
    const arrival_ref01_list = await arrival_ref01_ent.list(arrival_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['TRANSPORTREST_TRANSIT_APIS_TEST_ARRIVAL_ENTID']
