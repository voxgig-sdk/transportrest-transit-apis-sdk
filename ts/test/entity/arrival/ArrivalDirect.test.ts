...
describe('ArrivalDirect', async () => {
  ...
  test('direct-list-arrival', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-arrival', setup.live)) return
    if (skipIfMissingIds(t, setup, ["stop01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TRANSPORTREST_TRANSIT_APIS_TEST_ARRIVAL_ENTID']
